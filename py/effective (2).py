...
data=pd.read_excel(datafilenate)
...
n_ef = m.log10(Rk/rc)/m.log10(Rk/Rt)
for i in range(len(data['Q'])):
    Q=data['Q'][i]
    Rt_temp = (0.0134-1.6*0.000001*Lc)*(10**3*Q*m.sqrt(M*t/Kpriz))**(1/2)
    n_ef_temp = m.log10(Rk/rc)/m.log10(Rk/Rt_temp)
    print(n_ef_temp, " n_ef_temp")
    print(Q, " Q")
    if n_ef_temp > n_ef:
        n_ef=n_ef_temp
        print(n_ef, "n_ef")
